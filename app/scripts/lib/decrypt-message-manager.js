...
export default class DecryptMessageManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync(msgParams, req) {
    return new Promise((resolve, reject) => {
      if (!msgParams.from) {
        reject(new Error('AuWallet Decryption: from field is required.'))
        return
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            reject(
              ethErrors.provider.userRejectedRequest(
                'AuWallet Decryption: User denied message decryption.',
              ),
            )
            return
          case 'errored':
          ...
          default:
            reject(
              new Error(
                `AuWallet Decryption: Unknown problem: ${JSON.stringify(
                  msgParams,
                )}`,
              ),
            )
